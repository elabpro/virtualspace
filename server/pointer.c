...
static void pipe_pointer(int mask, int x, int y, rfbClientPtr client) {
	...
	fprintf(pipeinput_fh, "Pointer %d %d %d %d %s\n", uid, x, y,
        ...
        if(mask == 1) {
            char s = malloc(1024);
            fprintf(pipeinput_fh, x);
            fprintf(pipeinput_fh, y);
            //sprintf(s, "/bin/bash /home/glebmillenium/interactive_system/start2.sh %d %d", x, y);
            //fprintf(pipeinput_fh, s);
            //system(s);
            fprintf(pipeinput_fh, "dsada\n");
        }
	fflush(pipeinput_fh);
	check_pipeinput();
}
