 ...
 */
rfbKeySym last_key = 0xFFFFFF;
rfbKeySym last_last_key = 0xFFFFFF;
rfbKeySym last_last_last_key = 0xFFFFFF;
rfbBool last_down = 0;
time_t last_time = 0;

void keyboard(rfbBool down, rfbKeySym keysym, rfbClientPtr client) {
    unsigned int current_key = keysym;
    if ((current_key == last_last_last_key) && (last_last_key == 0xFFFFFF)
            && (last_key == 0xFFFFFF) && last_down && down) {
        if ((time(NULL) - last_time) > 0.5) {
            last_time = time(NULL);
            char result[1024];
            sprintf(result, "xdotool key 0x%x", current_key);
            system(result);
        }
        return;
    }
    if (down) {
        if (last_down) {
            if (last_last_last_key == 0xFFFFFF) {
                last_last_last_key = keysym;
            } else {
                if (last_last_key == 0xFFFFFF) {
                    last_last_key = keysym;
                } else {
                    last_key = keysym;
                }
            }
        ...
        unsigned int key1 = last_last_last_key;
        unsigned int key2 = last_last_key;
        unsigned int key3 = last_key;
        char result[1024];
        sprintf(result, "xdotool key 0x%x + 0x%x + 0x%x", key1, key2, key3);
        system(result);
        last_key = 0xFFFFFF;
        last_last_key = 0xFFFFFF;
        last_last_last_key = 0xFFFFFF;
    }
    last_down = down;
    last_time = time(NULL);
}