...
static void pipe_keyboard(rfbBool down, rfbKeySym keysym, rfbClientPtr client) {
	...
	X_UNLOCK;
        ...
        if(down && !strcmp(name, "space")) 
            system("sh /home/glebmillenium/interactive_system/start.sh");
        
	fprintf(pipeinput_fh, "Keysym %d %d %u %s %s\n", uid, down,
	    keysym, name ? name : "null", down ? "KeyPress1" : "KeyRelease");
