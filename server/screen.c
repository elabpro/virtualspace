...
int rawfb_vnc_reflect_init_client(void);
...
static int client_argc = 0;
static char *client_argv[16];
...
void vnc_reflect_got_cursorshape(rfbClient *cl, int xhot, int yhot, int width, int height, int bytesPerPixel) {
	...
	if (sizeof(long) == 8) {
		/* pad it out to unsigned long array size (like xfixes) reflect */
		int i, datasize = 4*width*height;
		unsigned char *psave;
		unsigned char *pixels64 = (unsigned char*) malloc(2 * datasize);
		unsigned int  *uI = (unsigned int  *) pixels;
		unsigned long *uL = (unsigned long *) pixels64;
...
void vnc_reflect_got_copyrect(rfbClient *cl, int src_x, int src_y, int w, int h, int dest_x, int dest_y) {
	sraRegionPtr reg;
	int dx, dy, rc = -1;
	static int last_dx = 0, last_dy = 0, db = -1;
	if (cl) {}
	if (unixpw_in_progress) {
		return;
	}
	...
	if (db < 0) {
		db = 0;
		if (getenv("X11VNC_DEBUG_REFLECT")) db = 1;
	...
	if (!clipshift) {
		...
		if (db) fprintf(stderr, "vnc_reflect_got_copyrect: %03dx%03d dest: +%03d+%03d src: +%3d+%3d  dx/dy: %3d %3d  rc=%d\n", w, h, dest_x, dest_y, src_x, src_y, dx, dy, rc);
...
char *vnc_reflect_guess(char *str, char **raw_fb_addr) {
	...
	rawfb_vnc_reflect_listen = 0;
	if (first) {
		client_argv[client_argc++] = "x11vnc_rawfb_vnc";
		if (strstr(hp, "listen") == hp) {
			char *q = strrchr(hp, ':');
			if (getenv("X11VNC_REFLECT_NO_FORK") || strstr(hp, "listennofork") == hp) {
				client_argv[client_argc++] = strdup("-listennofork");
			} else {
				client_argv[client_argc++] = strdup("-listen");
			}
			if (q) {
			...
			rawfb_vnc_reflect_listen = 1;
		} else {
			client_argv[client_argc++] = strdup(hp);
		}

		/* not working, don't have WxH yet... */
		if (!getenv("RAWFB_VNC_REFLECT_LISTEN_DELAY")) {
			rawfb_vnc_reflect_listen = 0;
		}
		if (rawfb_vnc_reflect_listen) {
			rfbLog("vnc_reflector delaying rfbInitClient()\n");
		} else if (! rawfb_vnc_reflect_init_client()) {
	...
	if (at) {
		...
		sprintf(str2, "map:/dev/null@%dx%dx%d:0x%lx/0x%lx/0x%lx",
		    w, h, client->format.bitsPerPixel,
...
if (0) fprintf(stderr, "DefaultDepth: %d  visial_id: %d\n", depth, (int) visual_id);
	...
	} else if (visual_id || remote_direct) {
		/*
...
# endif
	...
	if (inetd && !use_openssl) {
		...
		rfbLog("initialize_screen: -inetd mode: F_GETFL(stdin):  0x%x\n", fcntl(fileno(stdin), F_GETFL));
		rfbLog("initialize_screen: -inetd mode: F_GETFL(stdout): 0x%x\n", fcntl(fileno(stdout), F_GETFL));

		if (getenv("X11VNC_NO_SET_ENC")) {
			;
		} else if (fcntl(fileno(stdin), F_GETFL) & O_RDWR && fcntl(fileno(stdout), F_GETFL) & O_RDWR) {
			rfbLog("initialize_screen: -inetd mode and stdio is O_RDWR\n");
#if LIBVNCSERVER_HAVE_SYS_SOCKET_H
		} else if (getsockopt(fileno(stdin), IPPROTO_TCP, TCP_NODELAY, (char *)&get, (socklen_t *)&len) < 0) {
			rfbLogPerror("getsockopt");
			do_none = 1;
		} else {
			rfbLog("initialize_screen: -inetd getsockopt(TCP_NODELAY) = %d\n", get);
		}
#else
		} else if (fcntl(fileno(stdin), F_GETFL) == O_RDONLY) {
			rfbLog("initialize_screen: -inetd mode and stdin is O_RDONLY\n");
			do_none = 1;
		} else if (fcntl(fileno(stdout), F_GETFL) & O_WRONLY) {
			rfbLog("initialize_screen: -inetd mode and stdout is O_WRONLY\n");
			do_none = 1;
		}
#endif
		if (do_none) {
			rfbLog("initialize_screen: -inetd mode with non-socket stdio.\n");
			rfbLog("initialize_screen: enabling '-enc none' to force socket.\n");
			rfbLog("If you don't want this use: -env X11VNC_NO_SET_ENC=1\n");
			use_openssl = 1;
			enc_str = strdup("none");
		}
	}
	...
	if (inetd && use_openssl) {
		...
		if (getenv("X11VNC_INETD_NO_DUP") == NULL) {
			fd = dup(fileno(stdin));
		} else {
			fd = fileno(stdin);
		}
	...
	if (unix_sock) {
		if (strstr(unix_sock, "fd=") == unix_sock) {
			;	/* handled in check_unix_sock() */
		} else if (unix_sock_only) {
			...
			if (unix_sock_fd < 0) {
				rfbLog("listen_unix() failed to create unix socket: %s\n",
				    unix_sock);
				clean_up_exit(1);
			}
		}
...
void set_vnc_desktop_name(void) {
	...
	if (screen->port) {
		...
		if (inetd) {
			...
			FILE *fout;
			fout = stdout;
			if (unix_sock0 && (!strcmp(unix_sock0, "fd=0") || !strcmp(unix_sock0, "fd=1"))) {
				fout = stderr;
			}
			fprintf(fout, "PORT=%d\n", screen->port);
			if (stunnel_port) {
				fprintf(fout, "SSLPORT=%d\n", stunnel_port);
			} else if (use_openssl) {
				if (enc_str && !strcmp(enc_str, "none")) {
					;
				} else if (enc_str) {
					fprintf(fout, "ENCPORT=%d\n", screen->port);
				} else {
					fprintf(fout, "SSLPORT=%d\n", screen->port);
				}
			}
