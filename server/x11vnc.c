...
void terminal_services(char *list) {
	...
	while (1) {
		...
		nfd = select(fmax, &rd, NULL, NULL, &tv);
...
static void quick_pw(char *str) {
	 ...
	 *
	 * "%stdin2" means read user from first stdin line and pass from 2nd.
	 *
...
#	define CHECK_ARGC if (i >= argc-1) { \
	...
	for (i=1; i < argc; i++) {
		...
		if (!strcmp(arg, "-unixsock")) {
			...
			unix_sock0 = strdup(unix_sock);
			continue;
		}
		if (!strcmp(arg, "-unixsockonly")) {
			CHECK_ARGC
			unix_sock = strdup(argv[++i]);
			unix_sock0 = strdup(unix_sock);
			unix_sock_only = 1;
		...
		if (1) {
			...
			if (!strcmp(arg, "-rfbport") && i < argc-1) {
				...
				if (!strcasecmp(argv[i+1], "prompt")) {
					;
				} else if (!strcasecmp(argv[i+1], "auto")) {
					got_rfbport_force_auto = 1;
	...
	if (rawfb_vnc_reflect_listen) {
		/* this is not working yet... need to listen manually... */
		int i;
		rfbLog("now calling rawfb_vnc_reflect_init_client()\n");
		if (! rawfb_vnc_reflect_init_client()) {
			rfbLog("vnc_reflector failed.\n");
			clean_up_exit(1);
		}
		for (i=0; i<10; i++) {
